...
6. `ranked_keywords` with `domain: <site domain>`, `location_name: "<City>,<State>,United States"`, `language_name: "English"`, `limit: 200`. Say that this call spends quota. Keep rows with position 4 to 20. Drop brand terms. Sort by search volume descending. Take five.
...
- `ranked_keywords` returns nothing: ship the fixes alone and say the ranking data was empty for that location.
