...
3. `ranked_keywords` with `domain: <site domain>`, `location_name: "<City>,<State>,United States"`, `language_name: "English"`, `limit: 200`. This is the baseline: count of ranking terms, how many at positions 1 to 3, 4 to 10, 11 to 20.
4. `competitor_domains` with `domain: <site domain>`, `limit: 10`, plus `location_code` when the metro code is known. These are the organic rivals.
5. For each term the user named, five at most: `serp_rank_check` with `keyword: <term>`, `target_domain: <site domain>`, `depth: 100`, plus `location_code` when known. Record the position, the URL that ranks, the top three domains on that page, and the time of the call. When the user named no terms, skip this step.
6. Tell the user before steps 3 to 5 that these calls spend quota.

## Output

- Baseline: one line with the counts from step 3 and the `location_name` used.
...
- `ranked_keywords` empty: say the site has no ranking terms recorded for that location.
