...
get_my_tenant -> list_sites -> get_business_context -> list_issues (critical, limit 100) -> list_issues (high, limit 100) -> get_page (only to name the page for a chosen fix) -> ranked_keywords (location_name = metro, limit 200)
...
- [ ] The reply states the location_name passed to ranked_keywords.
